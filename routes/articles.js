...
const { postArticle, getArticleByTitle, getArticlesByUserId, getArticleById, updateArticle, getAllArticles, getBestArticles } = require("../models/articleModel");
...
// GET - api/articles, api/articles?title=<title>, api/articles?user_id=<user_id>
router.get("/", async (req, res) => {
  ...
  try {
    if (title) {
      ...
      const rows = await getArticlesByUserId(userId);
    ...
    console.error(error);
    return res.status(500).send("Error getting articles");
  }
});
